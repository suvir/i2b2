...
for data_file in data_files:
    ...
    file_length = len(output_dict['cad'])
    
    #Process label for each token as per the CRF models
    for i in range(file_length):
        label_list = []
        flag_label = False
        token = output_dict['cad'][i].split('\t')[0].strip()
        #print token
        if token == 'AZUREUS':
            #print "NEW LINE"
            final_result.write('\n')
            ...
            for k in output_dict:
                ...
                if this_label == 'O':
                    label_string = 'O'
                elif model_name in ['diabetes','cad','obese','hypertension','hyperlipidemia']:
                    label_string = '|'.join(this_label.split('-')) + '|indicator=mention'
                else:
                    label_string = '|'.join(this_label.split('-')) + '|indicator=' + model_name
                label_list.append(label_string)
            
            for label in label_list:
                if label.startswith('B') or label.startswith('I'):
                    final_result.write(token+'\t'+label+'\n')
                    flag_label = True
                    break
            if not flag_label:
